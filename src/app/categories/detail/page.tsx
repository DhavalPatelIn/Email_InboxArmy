...
export default async function CategoriesDetail() {
    return (
        <>
            <CategoryeScript>
                <main className="email-wrapper pb-10 md:py-16 md:pb-10 xl:py-24 desktopmode">
                                ...
                                <div className='bg-theme-light-gray-2 rounded-2xl'>
                                    <header className="email-header rounded-t-2xl w-full z-20 overflow-hidden">
                                        <div className='nav-header rounded-t-2xl border border-solid border-theme-border w-full p-2 md:p-4 flex items-center justify-between flex-wrap gap-4 bg-white'>
                                            ...
                                            <div className="code-row flex items-center">
                                                ...
                                                <button className="download-button text-xs md:text-base bg-transparent  md:hover:bg-theme-blue md:hover:text-white flex items-center justify-center px-0 md:px-4 py-1 md:py-2 rounded-lg whitespace-nowrap border-none left-button">
                                                    <span className='icon-wrap'>
                                    ...
                                    <div className="email-content-area rounded-b-2xl border border-solid border-theme-border p-16 min-h-screen">
                                        <div className='email-postdata bg-white'>Email Data</div>
                                        <div className='email-code bg-white hidden'>Email Code</div>
                                    </div>
                                </div>
                                ...
                                <div className='bg-white px-6 py-8 md:p-8 w-full md:rounded-2xl space-y-4'>
                                    <div className="flex items-center space-x-4">
                                        <div className="w-20 md:w-24 h-20 md:h-24 rounded-full overflow-hidden border border-solid border-theme-border flex items-center justify-center">
                                            <Image className='w-full h-full object-cover' src={CvleLogo} alt="Civile Logo" width={150} height={150} />
                                        </div>
                                        <h1 className="text-2xl font-bold">Civile</h1>
                                    </div>

                                    <div>
                                        <h3 className="text-base font-intersemi font-semibold pb-1">Subject lines:</h3>
                                        <p className="text-base md:text-lg">Last chance to shop 60% off</p>
                                    </div>

                                    <div>
                                        <h3 className="text-base font-intersemi font-semibold pb-1">Features:</h3>
                                        <div className="flex flex-wrap gap-2 mt-2">
                                            {['Clothing\'s', 'eCommerce', 'Fall'].map((feature, index) => (
                                                <span
                                                    key={index}
                                                    className="bg-theme-light-gray-2 text-base font-medium px-3 py-1 rounded-full"
                                                >
                                                    {feature}
                                                </span>
                                            ))}
                                    ...
                                    <div>
                                        <div className='flex items-center gap-3 border-t border-solid border-theme-border mt-6 pt-8'>
                                            <span>
                                                <Image src={Calander} alt="Civile Logo" width={18} height={20} />
                                            </span>
                                            <p className='text-lg'>April 10, 2025 6:13pm</p>
                                        </div>
                                    </div>
            ...
            <div className='py-10 md:pb-24'>
                <div className="container">
                    <h2 className="text-center pb-6 md:mb-12">Maybe you’ll also like…</h2>
                    <div className="flex flex-wrap justify-center gap-2 md:gap-3">
                        {categories.map((category) => (
                            <Link key={category} href={"/"} className="bg-white text-base font-medium px-4 py-2 rounded-full shadow-sm hover:bg-theme-blue hover:text-white transition ease-in-out duration-300 text-sm md:text-base" >{category}</Link>
                        ))}
                    </div>
